...
if(isset($_POST['submit']))
{
    ...
    $query = "UPDATE `student` SET `deleted`=?, `deleted_date`=? WHERE `id`=?";
    $stmt = $mysqli->prepare($query);
    ...
    // Bind parameters
    $stmt->bind_param("isi", $deleted, $deletedDate, $student_id);
    ...
    if ($result) {
        ...
        $_SESSION['success_message'] = "تم حذف هذا الطالب بنجاح.";
        include_once 'PopUpAlert.php';
        
    } else {
        $_SESSION['main_success_message'] = "خطأ في المعطيات";
        $_SESSION['success_message'] = "لم تتم ازالة هذا الطالب, حاول مرة اخرى";
        include_once 'PopUpAlert.php';
    }
...
<body>
        ...
        <form action="" method="post">
            ...
            <div class="contant">
                ...
                <div class="input-box">
                    ...
                    <input type="text" placeholder="البريد الالكتروني" name="email" id="email" disabled>
                </div>
                <div class="input-box">
                    <label for="phone">رقم الهاتف</label>
                    <input type="tel" placeholder="رقم الهاتف" name="phone" id="phone" disabled>
                </div>
                <div class="input-box">
                    <label for="password">كلمة السر</label>
                    <input type="password" placeholder="كلمة السر" name="password" id="password" disabled>
            ...
            <div class="button-container">
                <button name="submit" type="submit">ازالة الطالب</button>
            </div>
        </form>
    </div>
    ...
    <script>
        document.getElementById('student').addEventListener('change', function() {
            ...
            document.getElementById('email').value = selectedStudent['phone_number'];
            document.getElementById('phone').value = selectedStudent['email'];
